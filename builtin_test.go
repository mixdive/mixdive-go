...
func TestBuiltinEventConstructors(t *testing.T) {
	...
	client.Track(Login("google").SetEventUser("u1"))
	client.Track(SignUp("").SetId("sign-up-u2").SetEventUser("u2"))
	flushed(t, client)
	login := cap.bodies[0]
...
func TestPageViewConstructor(t *testing.T) {
	client, cap := trackServer(t)
	client.Track(PageView("/pricing").SetPropertyString("title", "Pricing"))
